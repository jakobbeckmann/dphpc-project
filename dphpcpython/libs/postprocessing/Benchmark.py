...
from dphpcpython.libs.paths import project_path
import plot_utils
import scipy.stats
...
class Benchmark:
    ...
    def plot_weak_scaling(self, save=False, show=False, file_name=None):
        ...
        # set up empty data container holding n cores, run times, speedup of max n cores
        algo_t_core = {algorithm: {'n_cores': [], 'run_times': [], 'time_1core': 0.0,
                                   'last_speedup': 0.0, 'errors': [], 'n_points': 0}
                       for algorithm in self.run_config['run_params']['algorithms']}
        ...
        for algorithm, algo_data in algo_t_core.iteritems():
            ...
            speedups = [algo_data['time_1core'] / run_times[n] for n in range(len(run_times)) if is_weak_scale_combo[n]]
            errors = algo_data['errors']
            errors = [[err[0] for err in errors], [err[1] for err in errors]]
            ...
            efficiency = [speedups[i] / cores_set[i] for i in range(len(speedups))]

            ax.plot(cores_set, efficiency, linewidth=3, label=algorithm, alpha=0.5)
            ax.plot(cores_set, efficiency, 'o', linewidth=3, alpha=0.3, color='black', markeredgecolor='none')
            # ax.errorbar(cores_set, efficiency, yerr=errors, fmt='none', alpha=0.5, color='black', capsize=3)

            print algorithm
        ax.plot([1, max_n_core], [1.0, 1.0], 'k--', linewidth=1, alpha=0.5, label='efficiency')
